 ...
 ** The first 32 bits are a control number generated by mp_alloc.
 ** The last 32 bits are the actual index of the block. */
static const uint64_t mp_mask_alive = 0xFFFFFFFF00000000;
...
static bool mp_is_alive(const struct mp_pool* pool, uint64_t handle) {
    ...
    return pool->blockc > index && alive && alive == (*mp_header_ptr(pool, index) & mp_mask_alive);
}
...
bool mp_alloc(struct mp_pool* pool, uint64_t* handle) {
    ...
    *(mp_header_ptr(pool, index)) = *handle;
    pool->blockc_in_use++;
    /* Erase payload. */
    memset(mp_payload_ptr(pool, index), 0, pool->block_size);
    return true;
}
